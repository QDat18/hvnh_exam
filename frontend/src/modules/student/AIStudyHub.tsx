...
import {
    ...
    AlertCircle, ChevronLeft, ChevronRight, Layers, Brain, BarChart2, Save
} from 'lucide-react';
...
const AIStudyHub: React.FC<AIStudyHubProps> = ({ subjectId }) => {
    ...
    const handleReview = async (quality: number) => {
        ...
        setIsReviewing(true);
        // Save locally to reviewMap instead of calling API immediately
    ...
    const handleSubmitSession = async () => {
        ...
        const reviewsToSubmit = Object.entries(reviewMap).map(([idx, quality]) => ({
            flashcardId: flashcards[parseInt(idx)].flashcardId,
            quality
        }));

        const learningCount = Object.values(reviewMap).filter(q => (q as number) <= 3).length;
        const knownCount = Object.values(reviewMap).filter(q => (q as number) >= 4).length;
        const timeSpent = Math.floor((Date.now() - sessionStartTime) / 1000);

        const payload = {
            documentId: selectedDoc?.studentDocId,
            subjectId: selectedDoc?.subjectId,
            totalCards: flashcards.length,
            knownCards: knownCount,
            learningCards: learningCount,
            timeSpentSeconds: timeSpent > 0 ? timeSpent : 0,
            reviews: reviewsToSubmit
        };

        // Save to cache before sending, in case token expires
        ...
        try {
            ...
            alert("Đã lưu kết quả phiên học thành công!");

            // Reload flashcards to get new mastery status
            setLoadingCards(true);
            const res = await studyHubApi.getFlashcards(selectedDoc!.studentDocId);
            setFlashcards(res.data?.flashcards || []);
            ...
            if (err.response?.status === 401) {
                // Ignore alert, Axios interceptor will redirect to login and payload is already in localStorage
            } else {
                alert("Có lỗi khi lưu kết quả. Vui lòng thử lại sau.");
            }
        }
    };
            ...
            <div className="row g-3 g-lg-4">
                ...
                <div className="col-12 col-lg-3 order-2 order-lg-1">
                    <div className="card border-0 shadow-sm rounded-4 overflow-hidden">
                        <div className="px-3 py-3 border-0 d-flex justify-content-between align-items-center" style={{ background: 'linear-gradient(135deg,#1e40af,#1e3a8a)' }}>
                            <div>
                            ...
                            <button className="btn btn-sm btn-link text-white p-0 d-lg-none" type="button" data-bs-toggle="collapse" data-bs-target="#libraryCollapse" aria-expanded="false" aria-controls="libraryCollapse">
                                <ChevronRight size={20} className="transition-all transform-90" />
                            </button>
                        </div>
                        <div className="collapse d-lg-block" id="libraryCollapse">
                ...
                <div className="col-12 col-lg-9 order-1 order-lg-2">
                    {!selectedDoc ? (
                        ...
                        <div className="card border-0 shadow-sm rounded-4 overflow-hidden d-flex flex-column" style={{ minHeight: 480 }}>
                            ...
                            {/* Tab nav */}
                            <div className="px-2 px-md-3 pt-3 pb-0 bg-white border-bottom d-flex justify-content-between align-items-center flex-wrap gap-2">
                                <span className="fw-semibold text-dark text-truncate d-none d-md-inline" style={{ maxWidth: 260, fontSize: '0.88rem' }}>{selectedDoc.documentTitle}</span>
                                <div className="d-flex gap-1 pb-2 pb-md-0 w-100 w-md-auto justify-content-center">
                                    {(['flashcard', 'chat', 'list'] as const).map(t => (
                                        <button key={t} onClick={() => setActiveTab(t)}
                                            className={`btn btn-sm fw-semibold rounded-pill px-2 px-md-3 border-0 flex-grow-1 flex-md-grow-0 ${activeTab === t ? 'btn-primary' : 'btn-light text-muted'}`}
                                ...
                                <div className="flex-grow-1 bg-light p-3 p-md-4 d-flex flex-column">
                                    {loadingCards ? (
                                        ...
                                        <div className="text-center py-4 animation-fade-in">
                                            ...
                                            <div className="d-flex justify-content-center gap-3 mb-4 flex-wrap">
                                                <button onClick={() => { setCurrentIdx(0); setIsFlipped(false); setSessionDone(false); setReviewMap({}); setSessionStartTime(Date.now()); }}
                                                    className="btn btn-light rounded-pill px-4 fw-bold shadow-sm text-primary border">
                                                ...
                                                <button onClick={handleSubmitSession}
                                                    ...
                                                    {isSubmitting ? <span className="spinner-border spinner-border-sm me-2" /> : <Save size={15} style={{ marginRight: 6 }} />}
                                        ...
                                        <div className="d-flex flex-column align-items-center mx-auto w-100" style={{ maxWidth: 580 }}>
                                            ...
                                            <div className="d-flex gap-2 w-100 flex-wrap"
                                                ...
                                                <div className="d-flex gap-2 w-100">
                                                    <ReviewBtn emoji="😵" label="Quên" color="#dc2626" quality={1} onClick={() => handleReview(1)} disabled={isReviewing} />
                                                    <ReviewBtn emoji="😕" label="Lờ mờ" color="#d97706" quality={2} onClick={() => handleReview(2)} disabled={isReviewing} />
                                                </div>
                                                <div className="d-flex gap-2 w-100">
                                                    <ReviewBtn emoji="😊" label="Nhớ" color="#0284c7" quality={4} onClick={() => handleReview(4)} disabled={isReviewing} />
                                                    <ReviewBtn emoji="🎯" label="Thuộc" color="#16a34a" quality={5} onClick={() => handleReview(5)} disabled={isReviewing} />
                                                </div>
                                            </div>
                                        </div>
                                    )}
